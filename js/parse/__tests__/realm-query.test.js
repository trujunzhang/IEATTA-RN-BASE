...
const realm = require('../realmObjects').default
...
describe("RealmQuery", function () {
    ...
    it('should able to add new criteria', function () {
        expect(query['criteria'].length).toEqual(0);
        query.addCriteria('id == 1');
        expect(query['criteria'].length).toEqual(1);
    })

    describe("equalTo", function () {
        it('equalTo number', function () {
            query.equalTo('id', 1001);
            expect(query.toString()).toEqual('id == 1001');
        });

        it('equalTo string', function () {
            query.equalTo('name', "username");
            expect(query.toString()).toEqual('name == "username"');
        });

        it('equalTo date', function () {
            const now = new Date();
            query.equalTo('createdAt', now);
            expect(query.toString()).toEqual('createdAt == ' + now.toString());
        });

        it('equalTo boolean', function () {
            query.equalTo('isDeleted', true);
            expect(query.toString()).toEqual('isDeleted == true');
        });
    });
    ...
    describe("beginsWith", function () {
        it('beginsWith not casing', function () {
            ...
            expect(query.toString()).toEqual('name BEGINSWITH "phu"');
        });
        it('beginsWith casing', function () {
            query.beginsWith('name', 'phu', true);
            expect(query.toString()).toEqual('name BEGINSWITH[c] "phu"');
        });
    });
    ...
    describe("endsWith", function () {
        it('endsWith not casing', function () {
            ...
            expect(query.toString()).toEqual('name ENDSWITH "phu"');
        });
        it('endsWith casing', function () {
            query.endsWith('name', 'phu', true);
            expect(query.toString()).toEqual('name ENDSWITH[c] "phu"');
        });
    });
    describe("contains", function () {
        it('contains not casing', function () {
            query.contains('name', 'phu');
            expect(query.toString()).toEqual('name CONTAINS "phu"');
        });
        it('contains casing', function () {
            query.contains('name', 'phu', true);
            expect(query.toString()).toEqual('name CONTAINS[c] "phu"');
        });
    });
    ...
    describe("between", function () {
        it('between number to number', function () {
            ...
            expect(query.toString()).toEqual('age >= 20 AND age <= 30');
        });
        it('between date to date', function () {
            const start = new Date();
            const end = new Date();
            query.between('createdAt', start, end);
            expect(query.toString()).toEqual(`createdAt >= ${start} AND createdAt <= ${end}`);
        });
    });
    ...
    describe("Basic comparation", function () {
        ...
        it("greaterThan number", function () {
            ...
            expect(query.toString()).toEqual('age > 20');
        });
    ...
    describe('In', function () {
        it('In array of number', function () {
            query.in('id', [1, 2])
            expect(query.toString()).toEqual('(id == 1 OR id == 2)');
        })
        it('In array of string', function () {
            query.in('status', ["ACTIVE", "DEACTIVE"])
            expect(query.toString()).toEqual('(status == "ACTIVE" OR status == "DEACTIVE")');
        });
        it('In array of mix values', function () {
            query.in('id', [1001, "abcd"])
            expect(query.toString()).toEqual('(id == 1001 OR id == "abcd")');
        });
    });
    ...
    describe('Query with compound criterias', function () {
        it('complex query with or', function () {
            ...
            expect(query.toString()).toEqual(expectedFilted);
        });
        ...
        it('combine complex query', function () {
            ...
            query.join(query2);
            expect(query.toString()).toEqual('name CONTAINS[c] "phu" AND (id == 1001 OR id == 1002)');
        });
    });
...
describe('Methods that RealmQuery not yet supported', function () {
    ...
    it('isEmpty', function () {
        expect(query.isEmpty).toThrow('Not yet supported "isEmpty');
    })
    it('isNotEmpty', function () {
        expect(query.isNotEmpty).toThrow('Not yet supported "isNotEmpty');
    })
    it('isNotNull', function () {
        expect(query.isNotNull).toThrow('Not yet supported "isNotNull');
    })
    it('isNull', function () {
        expect(query.isNull).toThrow('Not yet supported "isNull');
    })
    it('like', function () {
        expect(query.like).toThrow('Not yet supported "like');
    })
});