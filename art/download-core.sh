...
download_core() {
    ...
    if [ ! -f "$TAR" ]; then
	echo  "http://localhost:8111/downloads/$SERVER_DIR/$DOWNLOAD_FILE"
        curl -f -L -s "http://localhost:8111/downloads/$SERVER_DIR/$DOWNLOAD_FILE" -o "$TMP_TAR" ||
